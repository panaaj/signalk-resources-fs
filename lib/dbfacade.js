...
module.exports= {
        ...
        return new Promise( (resolve, reject)=> {
            if(config.settings.API) {
                Object.entries(config.settings.API).forEach( i=>{ 
                    if(i[1]) {
                        try {
                            let dbPath= path.join(this.savePath, `${i[0]}.db`)
                            this.resources[i[0]]= new PouchDB(dbPath)
                            this.resources[i[0]].info().then( info=> {
                                console.log(`${info.db_name} (${info.doc_count}) - OK...`)
                            }).catch(err=> { console.log(err) }); 
                            // ** create indexes **
                            /*if(i[0]=='waypoints' || i[0]=='notes') {
                                this.resources[i[0]].createIndex({
                                    index: {
                                        fields: ['position.latitude'], 
                                        name: 'latitude'
                                    }
                                }).catch(err=> { console.log(err) } )
                                this.resources[i[0]].createIndex({
                                    index: {
                                        fields: ['position.longitude'], 
                                        name: 'longitude'
                                    }
                                }).catch(err=> { console.log(err) } )                                
                            }
                            this.resources[i[0]].createIndex({
                                index: {
                                    fields: ['feature.geometry.coordinates'],
                                    name: 'coordinates'
                                }
                            }).catch(err=> { console.log(err) } )
                            
                            this.resources[i[0]].getIndexes().then( r=> {
                                console.log(`********************\n** ${i[0]}.db indexes **\n********************`)
                                Object.values(r.indexes).forEach(i=> console.log(` - ${i.name}`))
                            })*/
    ...
    getRecord: async function (db, uuid) {
        try {
            ...
            console.error(`Fetch ERROR: Resource ${uuid} could not be retrived!`)
            return err
        }
    },
